...
>
  <Button
    danger
    className="border-none p-0 m-0 font-medium"
  >
    Delete
  </Button>
</Popconfirm>
  ...
  {orderDetails ? orderDetails.orderId : "Đang tải..."}{" "}
</p>
<p>
  <strong>Tên khách hàng:</strong>{" "}
  {orderDetails ? orderDetails.buyerName : "Đang tải..."}
</p>
<p>
  <strong>Phone Number:</strong>{" "}
  {orderDetails ? orderDetails.phoneNumber : "Đang tải..."}
</p>
<p>
  <strong>Địa chỉ nhận hàng:</strong>{" "}
  {orderDetails ? orderDetails.shippingAddress : "Đang tải..."}
</p>
<p>
  <strong>Tổng Tiền:</strong>{" "}
  {orderDetails ? orderDetails.totalAmount.toFixed(3) : "Đang tải..."}
</p>
<p>
  <strong>Phương thức thanh toán:</strong>{" "}
  {orderDetails ? orderDetails.paymentMethod : "Đang tải..."}
</p>
</div>
<br />
<div>
  ...
  <div>
    <Drawer width={600} title="Đơn hàng đã đặt" onClose={onClose} open={open}>
          ...
          {orderDetails && orderDetails.orderItems
            ? orderDetails.orderItems.map((item, index) => (
                <tr key={index}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{item.price.toFixed(3)}</td>
                </tr>
              ))
            : null}
        </tbody>
...
<div className="w-1/3 border-l-2 border-gray-300 pl-6">
  {/* Status: Pending */}
  <div
    className={`flex items-center mb-6 ${orderDetails?.orderStatus === "pending" || orderDetails?.orderStatus === "completed" || orderDetails?.orderStatus === "shipped" ? "" : "opacity-50"}`}
  >
    <div className="ml-4">
      <p className="text-sm font-semibold">Pending</p>
      <p className="text-sm">
        {orderDetails ? new Date(orderDetails.createdAt).toLocaleTimeString() : "Loading..."} Ngày {orderDetails ? new Date(orderDetails.createdAt).toLocaleDateString() : "Loading..."}
      </p>
      <p className="text-sm">Status: Pending</p>
    </div>
  </div>
  <div
    className={`flex items-center mb-6 ${["completed", "shipped"].includes(orderDetails?.orderStatus) ? "" : "opacity-50"}`}
  >
    <div className="ml-4">
